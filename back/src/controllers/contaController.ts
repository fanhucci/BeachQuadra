...
export default class ContaController{
    ...
    async alterarStatus(req:Request, res:Response){
        ...
        const parse = AlterarStatusContaSchema.safeParse(req.params.id);
        
        if(!parse.sucess) return res.status(400).json({erro:"Id inválido"});
    ...
    async alterarSenha(req:Request, res:Response){
        ...
        return res.status(20).json(resposta);
    }
